...
class DataparserUnitTest(unittest.TestCase):
    ...
    def test_segment_message_single_segment(self):
        #write units tests for the segment_message method
        message = b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800||ADT^A03|||2.5'
        expected_segments = [b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800||ADT^A03|||2.5']
        result = self.obj.segment_message(message)
        self.assertEqual(result, expected_segments)

    def test_segment_message_ADT_A01(self):
        #write units tests for the segment_message method
        message = b'''MSH|^~\\&|SIMULATION|SOUTH RIVERSIDE|||20240102135300||ADT^A01|||2.5\rPID|1||497030||ROSCOE DOHERTY||19870515|M\r\r'''
        expected_segments = [b'MSH|^~\\&|SIMULATION|SOUTH RIVERSIDE|||20240102135300||ADT^A01|||2.5', 
    ...
    def test_segment_message_ADT_A03(self):
        #write units tests for the segment_message method
        message = b'''MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800||ADT^A03|||2.5\rPID|1||829339\r\r'''
        expected_segments = [b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800||ADT^A03|||2.5', 
                             b'PID|1||829339']
        result = self.obj.segment_message(message)
        self.assertEqual(result, expected_segments)
    ...
    def test_segment_message_ORU(self):
        #write units tests for the segment_message method
        message = b'''MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331113300||ORU^R01|||2.5\rPID|1||257406\rOBR|1||||||20240331113300\rOBX|1|SN|CREATININE||92.95579346699137\r\r'''
        expected_segments = [b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331113300||ORU^R01|||2.5',
                             b'PID|1||257406',
                             b'OBR|1||||||20240331113300',
                             b'OBX|1|SN|CREATININE||92.95579346699137']
        result = self.obj.segment_message(message)
        self.assertEqual(result, expected_segments)
    ...
    def test_segment_message_with_new_separator(self):
        ...
        expected_segments = [b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800||ADT^A03|||2.5',
                             b'PID|1||829339']
        result = self.obj.segment_message(message, segment=b'\n')
        self.assertEqual(result, expected_segments)

    ### For get_message_type function
    def test_get_message_type_ORU(self):
        message_segment = [b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331113300||ORU^R01|||2.5']
        expected_result = b'ORU^R01'
        result = self.obj.get_message_type(message_segment)
        self.assertEqual(result, expected_result)

    def test_get_message_type_ADT_A01(self):
        message_segment = [b'MSH|^~\\&|SIMULATION|SOUTH RIVERSIDE|||20240102135300||ADT^A01|||2.5']
        expected_result = b'ADT^A01'
        result = self.obj.get_message_type(message_segment)
        self.assertEqual(result, expected_result)

    def test_get_message_type_ADT_A03(self):
        message_segment = [b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800||ADT^A03|||2.5']
        expected_result = b'ADT^A03'
        result = self.obj.get_message_type(message_segment)
        self.assertEqual(result, expected_result)

    def test_get_message_type_invalid(self):
        message_segment = [b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800||ADT^A04|||2.5']
        with self.assertRaises(ValueError):
            self.obj.get_message_type(message_segment)

    def test_get_message_type_empty(self):
        message_segment = [b'MSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800|||||2.5']
        with self.assertRaises(ValueError):
            self.obj.get_message_type(message_segment)
    ...
    def test_process_message_ORU_R01(self):
        message = b'''\x0bMSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331113300||ORU^R01|||2.5\r
                    PID|1||257406\rOBR|1||||||20240331113300\rOBX|1|SN|CREATININE||92.95579346699137\r\x1c\r'''
        expected_result = b'ORU^R01'
        result = self.obj.process_message(message)
        self.assertEqual(result.message_type, expected_result)

    def test_process_message_ADT_A01(self):
        message = b'''\x0bMSH|^~\\&|SIMULATION|SOUTH RIVERSIDE|||20240102135300||ADT^A01|||2.5\r
                    PID|1||497030||ROSCOE DOHERTY||19870515|M\r\x1c\r'''
        expected_result = b'ADT^A01'
        result = self.obj.process_message(message)
        self.assertEqual(result.message_type, expected_result)

    def test_process_message_ADT_A03(self):
        message = b'''\x0bMSH|^~\&|SIMULATION|SOUTH RIVERSIDE|||20240331035800||ADT^A03|||2.5\r
                    PID|1||829339\r\x1c\r'''
        expected_result = b'ADT^A03'
        result = self.obj.process_message(message)
        self.assertEqual(result.message_type, expected_result)
